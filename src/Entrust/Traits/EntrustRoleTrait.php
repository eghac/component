...
trait EntrustRoleTrait
{
    ...
    {
        $rolePrimaryKey = $this->primaryKey;
        $cacheKey = 'entrust_modules_for_role_' . $this->$rolePrimaryKey;
        if (Cache::getStore() instanceof TaggableStore) {
            return Cache::tags(Config::get('entrust.role_module_table'))->remember($cacheKey, Config::get('cache.ttl', 60), function () {
                return $this->modules()->get();
            });
        } else return $this->modules()->get();
    }
...
//        return $this->belongsToMany(Config::get('entrust.module'), Config::get('entrust.role_module_table'));
    ...
    public function attachModule($module)
    {
        ...
        if (is_array($module)) {
            return $this->attachModules($module);
        }
    ...
    public function detachModule($module)
    {
        ...
        if (is_array($module)) {
            return $this->detachModules($module);
        }
    ...
    {
        if (!$modules) $modules = $this->modules()->get();
